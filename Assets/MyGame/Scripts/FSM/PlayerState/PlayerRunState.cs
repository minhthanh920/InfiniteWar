using System.Collections;
using UnityEngine;

public class PlayerRunState : State<Player>
{
    public PlayerRunState(BaseStateMachine<Player> stateMachine, Player character) : base(stateMachine, character) { }
    private float m_FootstepTimer = 0f;
    private float m_StaminaTimer = 0f;
    [SerializeField] private float m_FootstepInterval = 0.45f;
    [SerializeField] private float m_StaminaInterval = 1f;
    public override void Enter()
    {
        if (m_Character.m_Animator != null)
        {
            m_Character.m_Animator.SetBool(CONST.RUN, true);
        }
    }
    public override void Update()
    {
        if (m_Character.GetCurrentStamina() <= 0)
        {
            m_StateMachine.SetState(CharacterStateID.Walk);
        }
        //m_Character.OnRunAndWalk();
        if (Input.GetKeyUp(KeyCode.LeftShift))
        {
            m_Character.m_Input.sprint = false;
        }   
        if (m_Character.m_CharacterController.isGrounded)
        {
            
            m_FootstepTimer += Time.deltaTime;
            if (m_FootstepTimer >= m_FootstepInterval)
            {
                m_Character.OnFootStep();
                m_FootstepTimer = 0f;
            }
            m_StaminaTimer += Time.deltaTime;
            if (m_StaminaTimer >= m_StaminaInterval)
            {
                m_Character.RemainStamina(m_Character.m_RunCost);
                m_StaminaTimer = 0f;
            }
        }  
    }

    public override void Exit()
    {
        if (m_Character.m_Animator != null)
        {
            m_Character.m_Animator.SetBool(CONST.RUN, false);
        }
        m_Character.m_RootMotion = Vector3.zero;
    }
}

